...
//        drawYAxisArrow(canvas);
    ...
    protected void drawColumn(Canvas canvas) {
        if (columnList != null && columnColors != null) {
            ...
            for (int i = 0; i < columnList.size(); i++) {
                ...
                float leftTopY = (float) (originalY - (columnList.get(i) - minValue) * scale);
    ...
    protected void drawXAxisScaleValue(Canvas canvas) {
        ...
        for (int i = 0; i < xTitles.size() + 1 ; i++) {
            ...
            canvas.drawText(txt, originalX + dip2px(15) * i  + cellWidth * (i -1) - dip2px(10),
                    (float) (originalY + xTxtMargin *1.5),
                    mPaint);
            canvas.restore();
        }
    }
     ...
     */
    public void setColumnInfo(List<String> chartXTitles, List<Double> columnList, List<Integer> columnColors, int axisDivideSizeY) {
